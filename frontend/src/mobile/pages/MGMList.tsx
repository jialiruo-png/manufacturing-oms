...
const TABS = [
  ...
  { key: 'pending_ship_approval', label: '发货审批' },
  { key: 'procurement,production,ready_ship,shipped', label: '审批历史' },
];

const PAGE_SIZE = 20;

export default function MGMList({ user }: { user: User }) {
  ...
  const load = async (opts: { page: number; reset?: boolean }) => {
    ...
    try {
      const res = await ordersApi.listPaged({
        ...
        sort: statusKey.includes(',') ? 'updatedAt-desc' : undefined,
      });
  ...
  return (
    <MobileLayout title="审批管理" user={user} activeModule="gm">
      <div {...bindProps}>
        ...
        <div className="m-search-bar">
          <SearchOutlined />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="订单号 / 客户名" inputMode="search" onKeyDown={(e) => { if (e.key === 'Enter') void load({ page: 1, reset: true }); }} />
          {search && <a onClick={() => { setSearch(''); setTimeout(() => void load({ page: 1, reset: true }), 0); }} style={{ fontSize: 12, color: '#94a3b8' }}>清除</a>}
        </div>
        {loading ? <MLoading /> : orders.length === 0 ? <MEmpty text="暂无待审批订单" /> : (
          <>
            {orders.map((o) => {
              const daysLeft = getDaysLeft(o.deliveryDate);
              const dueClass = daysLeft < 0 ? 'danger' : daysLeft <= 3 ? 'warning' : '';
              const rail = o.urgent ? 'm-card-rail-red' : daysLeft < 0 ? 'm-card-rail-amber' : '';
              return (
                <div key={o.id} className={`m-order-card ${rail}`} onClick={() => push('gm', { id: o.id })}>
