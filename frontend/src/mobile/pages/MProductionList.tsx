...
const TABS = [
  ...
  { key: 'production', label: '在产' },
  { key: 'procurement', label: '缺料等待' },
];

const PAGE_SIZE = 20;

export default function MProductionList({ user }: { user: User }) {
  ...
  return (
    <MobileLayout title="生产管理" user={user} activeModule="production">
      <div {...bindProps}>
        ...
        {loading ? <MLoading /> : orders.length === 0 ? <MEmpty text="暂无生产订单" /> : (
          <>
            {orders.map((o) => {
              const daysLeft = getDaysLeft(o.deliveryDate);
              const dueClass = daysLeft < 0 ? 'danger' : daysLeft <= 3 ? 'warning' : '';
              const rail = daysLeft < 0 ? 'm-card-rail-red' : o.urgent ? 'm-card-rail-amber' : '';
              return (
                <div key={o.id} className={`m-order-card ${rail}`} onClick={() => push('production', { id: o.id })}>
